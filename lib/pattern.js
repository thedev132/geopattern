...
Pattern.prototype.geoHexagons = function () {
  ...
  Pattern.prototype.geoSineWaves = function () {
    ...
    var str, styles, xOffset;

    this.svg.setWidth(period * widthMultiplier);
    this.svg.setHeight(waveWidth * numWaves);
    ...
    for (let i = 0; i < numWaves; i++) {
        ...
        for (let j = 0; j < widthMultiplier; j++) {
            // Calculate a new val based on the current row AND repetition
            let valIndex = (i + j) % 36; // Cycle through the hash values
            let val = hexVal(this.hash, valIndex);
...
Pattern.prototype.geoChevrons = function () {
	...
	var canvasWidthMultiplier = 15;
	var canvasHeightMultiplier = 25;
  
	this.svg.setWidth(chevronWidth * canvasWidthMultiplier);
	this.svg.setHeight(chevronHeight * canvasHeightMultiplier * 0.66);
  
	i = 0;
	for (y = 0; y < canvasHeightMultiplier; y++) { 
	  for (x = 0; x < canvasWidthMultiplier; x++) { 
		val = hexVal(this.hash, i % this.hash.length);
		opacity = fillOpacity(val);
		fill = fillColor(val);
		...
		var groupX = x * chevronWidth;
		var groupY = y * chevronHeight * 0.66 - chevronHeight / 2;
  
		this.svg.group(styles).transform({
		  translate: [groupX, groupY]
		}).polyline(chevron).end();
  
		if (y === canvasHeightMultiplier - 1) {
		  this.svg.group(styles).transform({
			translate: [groupX, canvasHeightMultiplier * chevronHeight * 0.66 - chevronHeight / 2]
		  }).polyline(chevron).end();
		}
...
Pattern.prototype.geoXes = function () {
  ...
  Pattern.prototype.geoOverlappingCircles = function () {
	...
	var canvasWidthMultiplier = 18;
	var canvasHeightMultiplier = 20;
  
	this.svg.setWidth(radius * canvasWidthMultiplier);
	this.svg.setHeight(radius * canvasHeightMultiplier);
	...
	for (y = 0; y < canvasHeightMultiplier; y++) { 
	  for (x = 0; x < canvasWidthMultiplier; x++) { 
		...
		// Calculate circle position based on radius and current x and y
		var circleX = x * radius + radius / 2; // Center the circle within the grid
		var circleY = y * radius + radius / 2;
  
		this.svg.circle(circleX, circleY, radius, styles);
		i++;
	  }
	}
  };
...
Pattern.prototype.geoDiamonds = function () {
    ...
    var dx, fill, i, opacity, styles, val, x, y, w; 
    var widthMultiplier = 3; 

    const numCols = 10;
    const numRows = 40;

    this.svg.setWidth(diamondWidth * numCols * widthMultiplier); 
    this.svg.setHeight(diamondHeight * numRows);

    for (y = 0; y < numRows; y++) {
        for (w = 0; w < widthMultiplier; w++) { 
            for (x = 0; x < numCols; x++) {
                i = (y * numCols * widthMultiplier) + (w * numCols) + x; 
                val = hexVal(this.hash, i % 16); 
                opacity = fillOpacity(val);
                fill = fillColor(val);
                ...
                this.svg.polyline(diamond, styles).transform({
                    translate: [
                        (x + (w * numCols)) * diamondWidth - diamondWidth / 2 + dx, 
    ...
    //Tiling at the top and bottom
    for (w = 0; w < widthMultiplier; w++) { 
        for (x = 0; x < numCols; x++) {
            i = (numRows * numCols * widthMultiplier) + (w * numCols) + x;
            val = hexVal(this.hash, i % 16); 
            opacity = fillOpacity(val);
            fill = fillColor(val);
            ...
            dx = ((numRows) % 2 === 0) ? 0 : diamondWidth / 2;
            ...
            dx = ((-1) % 2 === 0) ? 0 : diamondWidth / 2;
             this.svg.polyline(diamond, styles).transform({
                translate: [
                    (x + (w * numCols)) * diamondWidth - diamondWidth / 2 + dx, 
                    diamondHeight / 2 * (-1) - diamondHeight / 2
                ]
            });
        }
    }
  ...
  Pattern.prototype.geoNestedSquares = function () {
    ...
    var fill, i, opacity, styles, val, x, y, w;
    var widthMultiplier = 3; 

    this.svg.setWidth(((squareSize + blockSize) * 6 + blockSize * 6) * widthMultiplier);
    ...
    for (y = 0; y < 6; y++) {
        for (w = 0; w < widthMultiplier; w++) { 
            for (x = 0; x < 6; x++) {
                ...
                this.svg.rect(currentX * squareSize + currentX * blockSize * 2 + blockSize / 2,
                    y * squareSize + y * blockSize * 2 + blockSize / 2,
                    squareSize, squareSize, styles);

                val = hexVal(this.hash, (39 - (i % this.hash.length)) % this.hash.length); 
                opacity = fillOpacity(val);
                fill = fillColor(val);
                ...
                this.svg.rect(currentX * squareSize + currentX * blockSize * 2 + blockSize / 2 + blockSize * 2,
...
Pattern.prototype.geoMosaicSquares = function () {
    ...
    var numTiles = 4 * sizeMultiplier;
    var widthMultiplier = 2; // Add this line - controls width extension
    var i, x, y;

    this.svg.setWidth(triangleSize * 2 * numTiles * widthMultiplier); // Multiply width
    this.svg.setHeight(triangleSize * 2 * numTiles);

    i = 0;
    for (y = 0; y < numTiles; y++) {
        for (let w = 0; w < widthMultiplier; w++) { // Loop for width extension
            for (x = 0; x < numTiles; x++) {
                var hashIndex = i % this.hash.length;
                let currentX = x + (w * numTiles); // calculate correct x position

                if (currentX % 2 === 0) {
                    if (y % 2 === 0) {
                        drawOuterMosaicTile(this.svg,
                            currentX * triangleSize * 2,
                            y * triangleSize * 2,
                            triangleSize,
    ...
    //The following code blocks are for creating the borders, these need to be adjusted as well
    // Add an extra column of tiles on the right
    for (y = 0; y < numTiles; y++) {
        var hashIndex = i % this.hash.length;
        if (y % 2 === 0) {
            drawInnerMosaicTile(this.svg,
                numTiles * triangleSize * 2 * widthMultiplier, //Corrected x position
                y * triangleSize * 2,
                triangleSize,
                [hexVal(this.hash, hashIndex), hexVal(this.hash, (hashIndex + 1) % this.hash.length)]
            );
    ...
    for (let w = 0; w < widthMultiplier; w++) { //Add loop for width extension
        for (x = 0; x < numTiles; x++) {
            var hashIndex = i % this.hash.length;
            let currentX = x + (w * numTiles);
            if (currentX % 2 === 0) {
                drawInnerMosaicTile(this.svg,
                    currentX * triangleSize * 2,
